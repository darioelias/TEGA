...
public class ProfesionalSpecifications {


    public static Specification<Profesional> findByCriteria(final ProfesionalCriterio criterio){

		return new SpecificationCountAbstract<Profesional>(){
			
            protected Predicate getPredicate(
                Root<Profesional> root,
                CriteriaQuery<?> query, CriteriaBuilder cb, Boolean fetch) {

				Join instituciones = getJoin(root,"instituciones",fetch);
				...
				if(criterio.getIdInstitucion() != null)
					predicates.add(cb.equal(instituciones.<Long>get("id"), criterio.getIdInstitucion()));
				...
				CriteriaQuery<Profesional> consulta = (CriteriaQuery<Profesional>) query;
