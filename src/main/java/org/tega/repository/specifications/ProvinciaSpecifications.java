...
public class ProvinciaSpecifications {
    ...
    public static Specification<Provincia> findByCriteria(final ProvinciaCriterio criterio){

		return new SpecificationCountAbstract<Provincia>(){
			
            protected Predicate getPredicate(
                Root<Provincia> root,
                CriteriaQuery<?> query, CriteriaBuilder cb, Boolean fetch) {

				Join pais = getJoin(root,"pais",fetch);
				...
				if(criterio.getIdPais() != null)
					predicates.add(cb.equal(pais.<Long>get("id"), criterio.getIdPais()));
