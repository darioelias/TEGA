...
public class LocalidadSpecifications {
    ...
    public static Specification<Localidad> findByCriteria(final LocalidadCriterio criterio){

		return new SpecificationCountAbstract<Localidad>(){
			
            protected Predicate getPredicate(
                Root<Localidad> root,
                CriteriaQuery<?> query, CriteriaBuilder cb, Boolean fetch) {

				Join provincia = getJoin(root,"provincia",fetch);
				...
				if(criterio.getIdProvincia() != null)
					predicates.add(cb.equal(provincia.<Long>get("id"), criterio.getIdProvincia()));
